...
from db.mongo import users_col, plans_col, orders_col, subscriptions_col

# ---- Users
async def get_or_create_user(tg_id: int, username: str | None, first_name: str | None):
    u = await users_col.find_one({"tg_id": tg_id})
    if u:
        await users_col.update_one({"_id": u["_id"]},
                                   {"$set": {"username": username, "first_name": first_name}})
        return u
    doc = {"tg_id": tg_id, "username": username, "first_name": first_name, "created_at": datetime.utcnow()}
...
async def mark_order_paid(order_id: ObjectId, provider: str, provider_ref: str):
    await orders_col.update_one(
        {"_id": order_id},
        {"$set": {"status": "paid", "provider": provider, "provider_ref": provider_ref, "paid_at": datetime.utcnow()}}
    )
